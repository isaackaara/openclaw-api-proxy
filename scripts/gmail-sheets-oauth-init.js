#!/usr/bin/env node
/**
 * gmail-sheets-oauth-init.js
 * One-time script to get a Gmail + Google Sheets OAuth refresh token.
 *
 * Prerequisites:
 ...
 *   4. Enable Gmail API AND Google Sheets API in APIs & Services -> Library
 *   5. Run: GMAIL_CLIENT_ID=xxx GMAIL_CLIENT_SECRET=yyy node scripts/gmail-sheets-oauth-init.js
 *
...
// Full Gmail access + Google Sheets access
const SCOPES = [
...
if (!CLIENT_ID || !CLIENT_SECRET) {
  ...
  console.error(
    "Usage: GMAIL_CLIENT_ID=xxx GMAIL_CLIENT_SECRET=yyy node scripts/gmail-sheets-oauth-init.js"
  );
  process.exit(1);
}
...
const server = http.createServer(async (req, res) => {
  ...
  res.end(`
    <h2>Gmail + Sheets Authorization Successful!</h2>
  ...
  try {
    ...
    const { access_token, refresh_token, expires_in, scope } = result.body;

    console.log("\n========================================");
    console.log("SUCCESS! Gmail + Sheets OAuth tokens obtained.");
    console.log("========================================\n");
    console.log("SCOPES GRANTED:");
    console.log(scope || "No scope returned (check token manually)");
    console.log("\nACCESS TOKEN (expires in " + expires_in + "s, auto-refreshed by proxy):");
    ...
    console.log("4. Test Sheets with:");
    console.log('   curl -H "Authorization: Bearer <proxy_token>" \\');
    console.log("        https://openclaw-api-proxy-production.up.railway.app/api/sheets/1n6ZRc3qvrlCtc2tiaFSMyIdZ0dlXQZLoanblI3o2ZKs/values/Receipts!A1:Z1");
    console.log("========================================\n");
...
server.listen(PORT, () => {
  console.log("========================================");
  console.log("Gmail + Sheets OAuth Init - Starting authorization flow");
  console.log("========================================");
  ...
  console.log("Grant Gmail AND Google Sheets access when prompted.");
